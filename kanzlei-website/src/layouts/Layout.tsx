...
export default function Layout({ lang }: LayoutProps) {
  ...
  // Get current language from prop (passed from App.tsx)
  const currentLang = lang || 'de';
  
  const footer = loadFooter(currentLang);
  const navLinks = loadNavItems(currentLang);
    ...
    <div className="min-h-screen flex flex-col overflow-x-hidden">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-40">
        <nav className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between" aria-label="Hauptnavigation">
          <Link to={lang ? '/en' : '/'} className="text-xl font-bold text-[#1a365d]" onClick={closeMobileMenu}>
            Kanzlei Recht
          </Link>
